...
function ExplosionConfetti({
  position = [0, 0, 0],
  colorsArray = ['#452632', '#91204d', '#e4844a', '#e8bf56', '#e2f7ce'],
  ...
  const data = useMemo(
    () =>
      Array.from({ length: limmitedParticleAmount }, () => ({
        color: colorsArray[Math.floor(Math.random() * 5)],
      })),
    [colorsArray]
  );
  ...
  return (
    <>
      {Array.from({ length: Math.min(numberOfExplosions, 3) }).map(
        (_, index) => (
          <Instances
            key={index}
          ...
          >
            ...
            {particles.map((data, i) => (
              <SingleParticle
                ...
                contettiDuration={
                  contettiDuration + Math.floor(Math.random() + 20 * index)
                }
